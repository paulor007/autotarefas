...
| R-16 | **Lacunas da CI — CONFIRMADO com os workflows locais (A1, 10/08/2026), risco mantido aberto**: a CI roda Python 3.12/3.13, `ruff check`, `ruff format --check`, `mypy src/`, bandit, `pytest` do núcleo e gate de cobertura, mas **não executa** `live_demo/backend/tests`, `npm ci`, `npm audit`, `npm audit --omit=dev`, `npm run typecheck`, `npm test`, `npm run build` nem Playwright real | inspeção de `.github/workflows/ci.yml` e `docs.yml` locais (idênticos aos do remoto) + `pyproject.toml` (`testpaths = ["tests"]`) | consolidação planejada na subetapa **A1.4** (`06`), com workflow de documentação mantido separado; hoje "CI verde" **não** significa Live backend e frontend verdes |
...
| R-18 | **22 dos 52 testes de frontend podem não estar preservados pelo Git**: `live_demo/frontend/src/lib/spreadsheets.test.ts` existe e executou 22 testes, mas **não apareceu em `git ls-files` nem entre os arquivos não rastreados**. Hipótese **não confirmada** de regra de `.gitignore` alcançando diretórios `lib` | saída local de `git ls-files` e `git status --short` (A1) | confirmar na **A1.2** com `git check-ignore -v` e `git ls-files --error-unmatch` antes de qualquer conclusão; se confirmado, corrigir a regra (com autorização) para que o arquivo seja rastreado |
