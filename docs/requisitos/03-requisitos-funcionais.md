...
**Testes:** `tests/tasks/test_extract_web_js.py` (unidade, browser mockado); `tests/e2e/test_extract_web_js_e2e.py` — na A1 (10/08/2026) media **1 passa e 1 auto-skipa** sem Chromium (`verify_playwright_installed`); com o navegador instalado, na homologação C1 (04/09/2026), **os 2 passam**.
**Prioridade:** OBRIGATÓRIO V1 (modo real). **Fase:** entregue (código); comprovação com navegador feita na Fase C1. **Status:** CONCLUÍDO em 04/09/2026 — o critério de aceite foi demonstrado com Chromium real.
**Evidências:** ramo `--js` em `extract_web.py`; `src/autotarefas/core/browser.py`; e a execução com navegador real registrada na matriz sob a marca **[C1]** — `pytest tests/e2e/test_extract_web_js_e2e.py -v --no-cov` com **2 passed** e nenhum *skipped*, provando por contraste que a mesma `/catalogo-js` rende 0 itens sem `--js` e os 3 produtos com `--js`. **Lacunas:** — a execução com navegador instalado foi feita; o caso com navegador é justamente o que auto-skipava, e agora passa. A ausência de execução pública com Chromium não é lacuna deste requisito: é a DP-02, que tirou o modo Live da V1 pública de propósito. **Próxima ação:** manter. **Demais campos:** padrão do módulo.
...
**Testes:** `tests/tasks/test_rpa_cadastro.py`; `tests/cli/test_rpa_cli.py`; `tests/core/test_browser.py` — com navegador **mockado**; e `tests/e2e/test_rpa_cadastro_e2e.py` — com **navegador real**, 6 casos, criado em 07/09/2026. A A1 (10/08/2026) havia constatado que não existia teste com navegador real para o RPA; `tests/e2e/` tinha só o caso de scraping com JavaScript. Deixou de ser verdade.
**Prioridade:** OBRIGATÓRIO V1 (modo real). **Fase:** entregue (código); comprovação com navegador feita na Fase C1. **Status:** CONCLUÍDO em 07/09/2026 — os dois critérios de aceite foram demonstrados com Chromium real.
**Evidências:** `src/autotarefas/tasks/rpa_cadastro.py`; e a execução com navegador real registrada na matriz sob a marca **[C2]** — 6 passed, nenhum *skipped*. Cada caso fecha lendo o `GET /cadastros` do **destino**, e não o retorno da task: verificar o retorno provaria que a task acha que cadastrou; ler o destino prova que cadastrou. **Lacunas:** — a decisão de 10/08/2026 exigia teste automatizado com navegador real antes de promover, e ele existe; a homologação manual segue complementar, como ela previa. **Próxima ação:** manter. **Demais campos:** padrão do módulo.
...
**Estado atual (CONCLUÍDO) — fechado em 03/09/2026:** o que a DP-05 pediu do lado do **código** já estava implementado e testado desde a conferência de 31/08/2026; as duas lacunas restantes eram documental e de interface, e ambas foram fechadas.
...
*Como as duas lacunas fecharam:* (i) **a política está escrita** em `SECURITY.md`, seção "Retenção de dados e privacidade" — os dois ambientes separados, cada prazo conferido no código antes de virar texto, os dois mecanismos de remoção (o comando explícito e a retenção automática do sink de log) e a ressalva de que é política técnica sujeita a revisão jurídica; (ii) **o Live passou a contar isso antes do upload**, pelo `apps/web/src/components/AvisoDePrivacidade.tsx`, renderizado imediatamente antes do `FileDrop` nos dois caminhos que existem (`ExecutionPanel.tsx` e `SpreadsheetJourney.tsx`), cobrindo os cinco itens que a DP-05 exige. Cumprir uma política que ninguém enunciou não era transparência; era coincidência — e agora quem envia o arquivo lê a política antes de enviá-lo, não depois.
...
**Prioridade:** OBRIGATÓRIO V1 (fecha o compromisso LGPD antes do release). **Fase:** A2. **Status:** CONCLUÍDO.
**Evidências do existente:** caminhos e testes citados no "Estado atual", mais `apps/web/src/components/AvisoDePrivacidade.test.tsx` (6 asserções, uma por item da DP-05) e `tests/e2e/test_jornada_do_visitante_e2e.py::test_15_sabe_o_que_acontece_com_o_arquivo_antes_de_envia_lo`, que confere em navegador real que o aviso está visível e **acima** da área de envio.
**Lacunas:** — as duas fecharam. (1) política documentada em `SECURITY.md`, seção "🗄️ Retenção de dados e privacidade"; (2) transparência no Live pelo `AvisoDePrivacidade`, antes do upload. Fora do escopo deste requisito e registrado na própria seção: a trilha de auditoria da **plataforma** nasceu depois da DP-05 e ainda não tem prazo definido.
**Próxima ação:** manter. O requisito era sobre o compromisso *declarado* com quem entrega os dados — a implementação sozinha nunca o fecharia, e é por isso que ele só fecha agora, com a política escrita e enunciada na tela.
