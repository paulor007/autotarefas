...
class TestValidateCliOutDir:
    def test_out_dir_gera_csvs_de_separacao(self, tmp_path: Path, cli_ctx: CLIContext) -> None:
        schema = tmp_path / "schema.yaml"
        schema.write_text(
            "columns:\n  - name: email\n    type: str\n    required: true\n    format: email\n",
            encoding="utf-8",
        ...
        result = runner.invoke(
            validate,
            [str(csv), "-s", str(schema), "--out-dir", str(out)],
            obj=cli_ctx,
        )
        # ha um e-mail invalido -> exit 1, mas os CSVs devem ser gerados
        assert result.exit_code == 1
        assert (out / "registros_validos.csv").exists()
        assert (out / "registros_invalidos.csv").exists()


class TestValidateCliOutDirXlsx:
    def test_out_dir_gera_planilha_xlsx(self, tmp_path: Path, cli_ctx: CLIContext) -> None:
        schema = tmp_path / "schema.yaml"
        schema.write_text(
            "columns:\n  - name: email\n    type: str\n    required: true\n    format: email\n",
            encoding="utf-8",
        ...
        runner.invoke(
            validate,
            [str(csv), "-s", str(schema), "--mode", "limpeza", "--out-dir", str(out)],
            obj=cli_ctx,
        )
        assert (out / "planilha_validada.xlsx").exists()
        assert (out / "registros_validos.csv").exists()
        assert (out / "registros_invalidos.csv").exists()


class TestValidateCliOutDirJson:
    def test_out_dir_gera_os_quatro_artefatos(self, tmp_path: Path, cli_ctx: CLIContext) -> None:
        schema = tmp_path / "schema.yaml"
        schema.write_text(
            "columns:\n  - name: email\n    type: str\n    required: true\n    format: email\n",
            encoding="utf-8",
        ...
        # os 4 artefatos do plano
        assert (out / "registros_validos.csv").exists()
        assert (out / "registros_invalidos.csv").exists()
        assert (out / "planilha_validada.xlsx").exists()
        assert (out / "validacao_report.json").exists()

    def test_json_contem_campos_consolidados(self, tmp_path: Path, cli_ctx: CLIContext) -> None:
        schema = tmp_path / "schema.yaml"
        schema.write_text(
            "columns:\n  - name: email\n    type: str\n    required: true\n    format: email\n",
            encoding="utf-8",
        ...
        report = json.loads((out / "validacao_report.json").read_text(encoding="utf-8"))
        assert report["mode"] == "limpeza"
        assert report["rows"] == 2
        assert report["total_valid"] == 1
        assert report["total_invalid"] == 1
        assert report["issues_by_category"].get("email") == 1
        # audit trail presente (Ana@Example.COM -> ana@example.com)
        assert report["total_cleaned"] >= 1
        assert any(c["after"] == "ana@example.com" for c in report["cleaning_changes"])
